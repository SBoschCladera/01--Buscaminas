...
class Juego {
    ...
    // Solicita al usuario el número de filas, columnas y la temática de las parejas para el tablero, cumpliendo unas ciertas condiciones.
    pedirDatosUsuario() {

        // Solicita número de filas y columas mediante prompt.
        this.inputDatoUsuario();

        // Comprueba si el número de casillas del tablero es correcto (debe ser un número par) y debe contener un mínimo de 2 parejas.
...
class Memory extends Tablero {
    ...
    // por el usuario colocadas en posiciones aleatorias. Las 10 primeras parejas siempre serán diferentes, comienzan a repetirse a partir
    // de la pareja 11. 
    arrayDeParejas() {
    ...
    // Esta función devuelve un número aleatorio entre 0 (incluido) y 1 (excluido), por lo que al restarle 0.5 se generan número positivos y 
    // negativos de forma aleatoria (entre -0.5 y 0.49999). De esta manera la función reoordena el array colocando un elemento delante o detrás 
    // de otro de forma aleatoria.
    ...
    pintarTableroEnPantalla() {
        ...
        this.entradaTabla(4000);
        setInterval(entradaTabla, segundos)
    }

    // Introduce un efecto de maquetación en el CSS para el título (h1)
    entradaTabla(segundos) {
